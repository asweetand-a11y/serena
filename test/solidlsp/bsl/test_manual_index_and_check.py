...
def test_manual_index_and_check_procedure():
    ...
    print(f"\n{'=' * 80}")
    print("Ручной запуск индексации и проверка кеша")
    print(f"{'=' * 80}")
    ...
    while elapsed < max_wait:
        ...
        if ls_manager is not None:
            print("[OK] Language server manager инициализирован")
            break
        ...
        if bsl_files:
            ...
            try:
                ...
                results = bsl_ls._index_files_with_local_parser(
                    file_paths=rel_paths, save_cache_callback=lambda: ls_manager.save_all_caches()
                )
    ...
    try:
        ...
        print("[OK] Кеш сохранен")
    except Exception as e:
        print(f"[WARNING] Ошибка при сохранении кеша: {e}")

    # Проверяем кеш
    print(f"\n{'=' * 80}")
    print("Проверка кеша после индексации")
    print(f"{'=' * 80}")
            ...
            if doc_symbols and doc_symbols.root_symbols:
                ...
                def search_symbols(symbols, depth=0):
                    ...
                    for symbol in symbols:
                        ...
                        # Более гибкий поиск
                        if (
                            procedure_name.lower() in symbol_name.lower()
                            or symbol_name.lower() in procedure_name.lower()
                            or "ОграничениеТипов" in symbol_name
                            or "АналитикПремирования" in symbol_name
                        ):
                ...
                print("  [WARNING] Символы отсутствуют в файле")
