...
class Project(ToStringMixin):
    ...
    def gather_source_files(self, relative_path: str = "") -> list[str]:
        ...
        # Оптимизация: предварительно собираем все расширения файлов для быстрой проверки
        # Вместо вызова fnmatch для каждого файла, проверяем расширение напрямую
        source_extensions: set[str] = set()
        for language in self.project_config.languages:
            fn_matcher = language.get_source_fn_matcher()
            for pattern in fn_matcher.patterns:
                # Извлекаем расширение из паттерна (например, "*.py" -> ".py")
                if pattern.startswith("*."):
                    ext = pattern[1:]  # ".py"
                    source_extensions.add(ext.lower())
                elif pattern.startswith("."):
                    source_extensions.add(pattern.lower())
        
        # Кэш для проверки расширений файлов
        _extension_cache: dict[str, bool] = {}
        
        def _is_source_file_by_extension(filename: str) -> bool:
            """Быстрая проверка расширения файла через кэш."""
            if not source_extensions:
                return True  # Если нет языков, считаем все файлы исходными
            
            filename_lower = filename.lower()
            if filename_lower in _extension_cache:
                return _extension_cache[filename_lower]
            
            # Проверяем все расширения
            result = any(filename_lower.endswith(ext) for ext in source_extensions)
            _extension_cache[filename_lower] = result
            return result
        
        # Оптимизированная реализация с os.scandir() для лучшей производительности
        # os.scandir() быстрее os.walk() так как использует меньше системных вызовов
        # и кэширует информацию о файлах (stat) в os.DirEntry
        def scan_directory_recursive(dir_path: str, followlinks: bool = True) -> None:
            """
            Рекурсивно сканирует директорию используя os.scandir() для максимальной производительности.
            
            :param dir_path: Путь к директории для сканирования
            :param followlinks: Следовать ли символическим ссылкам (по умолчанию True, как в os.walk)
            """
            try:
                    ...
                    for entry in entries:
                        try:
                            ...
                            if entry.is_file(follow_symlinks=followlinks):
                                # Быстрая проверка расширения перед проверкой игнорирования
                                if not _is_source_file_by_extension(entry.name):
                                    continue
                                
                                # Вычисляем относительный путь
                                try:
                                    rel_file_path = os.path.relpath(entry.path, start=self.project_root)
                                except Exception:
                                    log.warning(
                                ...
                                if not self.is_ignored_path(rel_file_path, ignore_non_source_files=False):
                                    rel_file_paths.append(rel_file_path)
                                else:
                                    log.debug(f"Ignoring file: {rel_file_path}")
                            
                            # Рекурсивно обрабатываем директории
                            elif entry.is_dir(follow_symlinks=followlinks):
                                # Проверяем, не игнорируется ли директория перед рекурсивным обходом
                                try:
                                    rel_dir_path = os.path.relpath(entry.path, start=self.project_root)
                                    if self.is_ignored_path(rel_dir_path):
                                        log.debug(f"Ignoring directory: {rel_dir_path}")
                                        continue
                                except (FileNotFoundError, OSError) as e:
                                    log.debug(f"Error checking directory {entry.path}: {e}, ignoring it")
                                    continue
                                
                                # Рекурсивно сканируем поддиректорию
                                scan_directory_recursive(entry.path, followlinks=followlinks)
                ...
                log.debug(f"Skipping directory {dir_path} due to error: {e}")
