            ...
            def run_task() -> None:
                try:
                    if self.future.done():
                        ...
                        return
                    if self.logged:
                        log.debug(f"Task {self.name} starting execution in thread")
                    with LogTime(self.name, logger=log, enabled=self.logged):
                        result = self._function()
                        if not self.future.done():
                            self.future.set_result(result)
                            if self.logged:
                                log.debug(f"Task {self.name} completed successfully, result set")
    ...
    def _process_task_queue(self) -> None:
        while True:
            ...
            if task.logged:
                ...
                log.debug("Starting execution of %s (not logged)", task.name)
            task.start()

            # wait for task completion
            if task.logged:
                log.debug("Waiting for task %s to complete (timeout: %s)", task.name, task.timeout)
