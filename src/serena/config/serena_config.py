...
class ProjectConfig(ToolInclusionDefinition, ToStringMixin):
    ...
    def _from_dict(cls, data: dict[str, Any]) -> Self:
        ...
        for language_str in data["languages"]:
            ...
            try:
                # Accept common misconfigurations where language is provided as a mapping (e.g., {bsl: {}})
                if isinstance(language_str, dict):
                    # Use the first key as language name
                    if len(language_str) == 0:
                        raise ValueError("Empty language mapping")
                    language_str = next(iter(language_str.keys()))
