...
class GitignoreParser:
    ...
    def __init__(self, repo_root: str) -> None:
        ...
        # Кэш для результатов should_ignore для директорий (оптимизация производительности)
        self._should_ignore_cache: dict[str, bool] = {}
    ...
    def _has_non_empty_patterns(self, gitignore_path: str) -> bool:
        """
        Быстрая проверка, содержит ли .gitignore файл непустые паттерны (не только комментарии и пустые строки).
        Это позволяет избежать сканирования поддиректорий, если в .gitignore нет паттернов.
        
        :param gitignore_path: Путь к .gitignore файлу
        :return: True если файл содержит непустые паттерны, False иначе
        """
        try:
            with open(gitignore_path, encoding="utf-8") as f:
                for line in f:
                    ...
                    # Пропускаем комментарии (но не экранированные)
                    line_stripped = line.lstrip()
                    if line_stripped.startswith("#") and not line_stripped.startswith("\\#"):
                        continue
                    
                    # Обрабатываем negation и экранированные символы
                    # Удаляем negation символ если есть (но не экранированный)
                    if line_stripped.startswith("!") and not line_stripped.startswith("\\!"):
                        line_stripped = line_stripped[1:].lstrip()
                    
                    # Обрабатываем экранированные символы в начале
                    if line_stripped.startswith("\\#") or line_stripped.startswith("\\!"):
                        line_stripped = line_stripped[1:]
                    
                    # Проверяем, осталось ли что-то после всех обработок
                    if line_stripped.strip():
                        return True
        except (OSError, UnicodeDecodeError):
            # Если не можем прочитать файл, считаем что он может содержать паттерны
            return True
        return False

    def _iter_gitignore_files(self, follow_symlinks: bool = False) -> Iterator[str]:
        """
        Оптимизированный поиск .gitignore файлов в репозитории.
        
        Оптимизации:
        - Проверяет содержимое .gitignore перед сканированием поддиректорий
        - Использует кэш для результатов should_ignore
        - Избегает лишних вызовов os.path.exists/os.path.isdir
        
        :return: итератор путей к .gitignore файлам (сверху вниз)
        """
        queue: list[str] = [self.repo_root]
        visited_dirs: set[str] = set()

        def scan(abs_path: str) -> Iterator[str]:
            """Сканирует директорию и возвращает найденные .gitignore файлы."""
            try:
                for entry in os.scandir(abs_path):
                    try:
                        if entry.is_dir(follow_symlinks=follow_symlinks):
                            entry_path = entry.path
                            # Избегаем повторного посещения директорий
                            if entry_path not in visited_dirs:
                                ...
                                if abs_path != self.repo_root:
                                    rel_path = os.path.relpath(entry_path, self.repo_root)
                                    if self._should_ignore_cached(rel_path, is_dir=True):
                                        continue
                                queue.append(entry_path)
                        elif entry.is_file(follow_symlinks=follow_symlinks) and entry.name == ".gitignore":
                            gitignore_path = entry.path
                            # Проверяем, есть ли в файле паттерны перед добавлением в очередь поддиректорий
                            if self._has_non_empty_patterns(gitignore_path):
                                yield gitignore_path
                    except (OSError, PermissionError) as e:
                        # Пропускаем файлы/директории, к которым нет доступа
                        log.debug(f"Skipping entry {entry.path} due to error: {e}")
                        continue
            except (OSError, PermissionError) as e:
                log.debug(f"Skipping directory {abs_path} due to error: {e}")

        while queue:
            next_abs_path = queue.pop(0)
            yield from scan(next_abs_path)

    def _should_ignore_cached(self, rel_path: str, is_dir: bool = False) -> bool:
        """
        Кэшированная версия should_ignore для оптимизации производительности.
        Использует кэш для директорий, чтобы избежать повторных проверок.
        
        :param rel_path: Относительный путь от корня репозитория
        :param is_dir: True если это директория (избегает os.path.exists/os.path.isdir)
        :return: True если путь должен быть проигнорирован
        """
        # Используем кэш только для директорий
        if is_dir and rel_path in self._should_ignore_cache:
            return self._should_ignore_cache[rel_path]
        
        result = self.should_ignore(rel_path)
        
        # Кэшируем результат для директорий
        if is_dir:
            self._should_ignore_cache[rel_path] = result
    ...
    def should_ignore(self, path: str) -> bool:
        ...
        # Ignore paths inside .git (быстрая проверка без os.path.exists)
        if rel_path:
        ...
        # Оптимизация: проверяем os.path.exists/os.path.isdir только если есть ignore_specs
        # и только если нужно определить, это директория или файл
        if self.ignore_specs:
            abs_path = os.path.join(self.repo_root, rel_path)
            # Проверяем, является ли путь директорией, только если нужно
            # (для правильного добавления "/" в конце)
            try:
                if os.path.exists(abs_path) and os.path.isdir(abs_path) and not rel_path_normalized.endswith("/"):
                    rel_path_normalized = rel_path_normalized + "/"
            except OSError:
                # Если не можем проверить, продолжаем без добавления "/"
                pass

        # Check against each ignore spec
        for spec in self.ignore_specs:
            if spec.matches(rel_path_normalized):
                return True
    ...
    def reload(self) -> None:
        ...
        self._should_ignore_cache.clear()
        self._load_gitignore_files()
