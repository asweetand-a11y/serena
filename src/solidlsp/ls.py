...
class SolidLanguageServer(ABC):
    ...
    def _load_document_symbols_cache(self) -> None:
        cache_file = self.cache_dir / self.DOCUMENT_SYMBOL_CACHE_FILENAME
        log.debug(f"Checking for document symbols cache at: {cache_file}")
        log.debug(f"Cache directory exists: {self.cache_dir.exists()}")
        if cache_file.exists():
            log.info("Loading document symbols cache from %s", cache_file)
            try:
                ...
                if saved_cache is not None:
                    ...
                    log.warning(f"Cache file exists but load_cache returned None (version mismatch or corrupt): {cache_file}")
            except Exception as e:
                # cache can become corrupt, so just skip loading it
                log.warning(
            ...
            log.debug(f"Document symbols cache file does not exist: {cache_file}")
