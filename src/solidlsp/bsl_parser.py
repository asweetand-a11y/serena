...
class BSLParser:
    ...
    PARAM_PATTERN = re.compile(r"(?:Знач\s+)?([а-яёА-ЯЁ\w]+)(?:\s*=\s*([^,)]+))?", re.IGNORECASE)

    # Переменные модуля: Перем Имя [Экспорт];
    MODULE_VAR_PATTERN = re.compile(r"^\s*Перем\s+([а-яёА-ЯЁ\w]+)(?:\s+Экспорт)?\s*;", re.IGNORECASE | re.MULTILINE)

    # Вызовы процедур/функций: ИмяПроцедуры( или ИмяПроцедуры()
    CALL_PATTERN = re.compile(r"\b([а-яёА-ЯЁ\w]+)\s*\(", re.IGNORECASE)
    ...
    def _parse_module_vars(self, source: str, lines: list[str]) -> dict[str, BSLModuleVar]:
        ...
        for match in self.MODULE_VAR_PATTERN.finditer(source):
            ...
            vars_dict[var_name] = BSLModuleVar(name=var_name, is_export=is_export, description=description)
    ...
    def _parse_method_from_match(self, source: str, lines: list[str], match: re.Match[str], is_proc: bool) -> BSLMethod | None:
    ...
    def _extract_params(self, source: str, start_pos: int, lines: list[str], start_line: int) -> list[BSLParam]:
    ...
    def _parse_global_calls(self, source: str, lines: list[str], methods: list[BSLMethod]) -> list[BSLCallPosition]:
    ...
    def _parse_method_calls(self, source: str, lines: list[str], method: BSLMethod) -> list[BSLCallPosition]:
